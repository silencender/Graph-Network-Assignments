    ...
    for i in tqdm(range(1, n)):
        for j in range(0, i):
            if ec[i][j] == N:
                ...
                ava_len = len(ava_idx) + sum(
                    [i not in ava_idx, j not in ava_idx])
                mc = [{
                    'nxt': np.zeros((ken), dtype=int),
                    'drop': np.zeros((ken), dtype=int)
                } for i in range(ncolors)]
    ...
    for i in tqdm(range(n - 3)):
        for j in range(i + 1, n - 2):
            for q in range(j + 1, n - 1):
                for w in range(q + 1, n):
                    al = ec[i][j] + ec[i, q] + ec[i, w] + ec[j, q] + ec[
                        j, w] + ec[q, w]
                    if al == 6 or al == (6 << 3):
                        kn += 1
