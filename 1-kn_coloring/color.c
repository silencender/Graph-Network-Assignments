    ...
    for(int i=0; i<n-3; i++) {
        for(int j=i+1; j<n-2; j++) {
            for(int p=j+1; p<n-1; p++) {
                for(int q=p+1; q<n; q++) {
                    int al = edge_record[i][j] + edge_record[i][p]+ edge_record[i][q]+ edge_record[j][p]+ edge_record[j][q]+ edge_record[p][q];
                    if(al == 6 || al == (6<<3)) {
                        kn++;
                    }
                }
                        ...
                        for(int b=a+1; b<n; b++) {
                            if(b!=i && b!=j) {
                                for(int c=0; c<ncolors; c++) {
                                    ...
                                    dp_move(color_map, color_map[c], edge_colors, masks, bin_len, ncolors, dp_record[c][0], dp_record[c][1]);
                                }
                            }
                        }
                    }
